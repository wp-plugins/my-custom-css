...
You can see in action (view source code) here: http://vegamami.altervista.org/ :)
...
if (!defined('DB_NAME')) {
	die('<h1>Ops: <a href="http://www.darkwolf.it/">http://www.darkwolf.it/</a></h1>');
}

function add_my_custom_css() {
	$mycustomcss = get_option('my_custom_css');
	if (empty($mycustomcss)) { $mycustomcss = '/* No Custom CSS Set */'; }
		echo "\n<!-- My Custom CSS Start -->\n<style type=\"text/css\">\n/* Plugin Author: Salvatore Noschese\nDarkWolf: http://www.darkwolf.it/ */\n\n".$mycustomcss."\n</style>\n<!-- My Custom CSS End -->\n";
}

function mccss_admin() {
	$plugin_page = add_options_page(__('My Custom CSS Panel','mccss'),__('My Custom CSS','mccss'), 'manage_options', 'my_custom_css', 'mccss_options');
